...
import SeampInvoiceContent from './SeampInvoiceContent'

export default function SeampPageContent() {
  ...
  useEffect(() => {
    const fetchQuotation = async () => {
      try {
        ...
        logQuotationPayloadForUrlId(id, quotation, 'seamp')
    ...
    <main className="quotation-doc" style={{ padding: '8px' }}>
      ...
      {error && (
        <div style={{ textAlign: 'center', padding: '40px', color: '#d32f2f' }}>
          <div style={{ fontWeight: 'bold', marginBottom: '8px' }}>Error loading Seamp Invoice</div>
          <div>{error}</div>
