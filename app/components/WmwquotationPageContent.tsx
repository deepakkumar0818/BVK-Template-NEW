...
import WmwquotationInvoiceContent from './WmwquotationInvoiceContent'

export default function WmwquotationPageContent() {
  ...
  useEffect(() => {
    const fetchQuotation = async () => {
      try {
        ...
        logQuotationPayloadForUrlId(id, quotation, 'wmwquotation')
    ...
    <main className="quotation-doc" style={{ padding: '8px' }}>
      ...
      {error && (
        <div style={{ textAlign: 'center', padding: '40px', color: '#d32f2f' }}>
          <div style={{ fontWeight: 'bold', marginBottom: '8px' }}>Error loading WMW Quotation</div>
          <div>{error}</div>
