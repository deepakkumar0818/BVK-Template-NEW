...
import EveriteInvoiceContent from './EveriteInvoiceContent'

export default function EveritePageContent() {
  ...
  useEffect(() => {
    const fetchQuotation = async () => {
      try {
        ...
        logQuotationPayloadForUrlId(id, quotation, 'everite')
    ...
    <main className="quotation-doc" style={{ padding: '8px' }}>
      ...
      {error && (
        <div style={{ textAlign: 'center', padding: '40px', color: '#d32f2f' }}>
          <div style={{ fontWeight: 'bold', marginBottom: '8px' }}>Error loading Everite Invoice</div>
          <div>{error}</div>
