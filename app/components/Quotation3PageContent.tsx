...
import Quotation3InvoiceContent from './Quotation3InvoiceContent'

export default function Quotation3PageContent() {
  ...
  useEffect(() => {
    const fetchQuotation = async () => {
      try {
        ...
        logQuotationPayloadForUrlId(id, quotation, 'quotation3')
    ...
    <main className="quotation-doc" style={{ padding: '8px' }}>
      ...
      {error && (
        <div style={{ textAlign: 'center', padding: '40px', color: '#d32f2f' }}>
          <div style={{ fontWeight: 'bold', marginBottom: '8px' }}>Error loading Quotation3</div>
          <div>{error}</div>
