...
interface EkamassGoodsTableProps {
  data: QuotationData
...
export default function EkamassGoodsTable({ data, rawQuotationData, shippingData, headerNode, footerNode }: EkamassGoodsTableProps) {
  ...
  const ekamassChargeRows: readonly [string, number][] = filterNonZeroWmwChargeRows([
  ...
  const lineItemsFromZoho = rawLineItems.map((item, index) => {
    ...
    // Ekamass: Product must come from Category_1_MM_Database_WMW_2_0 (2.0) Blend_Category only.
    ...
    /** Product row: Ekamass requires Blend_Category only; if missing, keep it blank. */
  ...
  if (displayLineItems.length === 0) {
    displayLineItems = [
      { item: 1, product: '', form: 'Endless Diagonal Seam', quality: 'AISI 316L', hsnCode: '7314', mesh: '40/ Inch', brand: 'Formx-040', size: '4.728 x 3.020', sqmArea: '14.2786', quantity: 6, delivery: '', rate: 1070, amount: 6420, perPc: 23.0, totalWeight: 138.0 },
      { item: 2, product: '', form: 'Endless Diagonal Seam', quality: 'AISI 316L', hsnCode: '7314', mesh: '40/ Inch', brand: 'Formx-040', size: '4.720 x 3.020', sqmArea: '14.2544', quantity: 3, delivery: '', rate: 1065, amount: 3195, perPc: 22.0, totalWeight: 66.0 }
    ]
  }
        ...
        return (
          ...
          >
            <div className="quotation-seamless-stack">
              ...
              <table
                className="goods-description-table quotation-stack-table ekamass-goods-table"
                style={{
                  width: '100%',
              ...
              >
                ...
                <tbody>
                  <tr className="ekamass-goods-title-row">
                  ...
                  {chunk.map((row, index) => (
                    <Fragment key={`ekamass-line-${pageIdx}-${index}`}>
                      <tr className="ekamass-item-meta-row">
                      ...
                      <tr className="ekamass-item-grid-row">
                  ...
                  {isLastChunk && (
                    <>
                      <tr aria-hidden className="ekamass-goods-spacer">
                      ...
                      <tr className="ekamass-inside-quotation-row">
                        <td
                          colSpan={2}
                          style={{
                            ...bdSides,
                      ...
                      {ekamassChargeRows.map(([chargeLabel, chargeAmt], chargeIdx) => (
                        <tr key={`ekamass-charge-${chargeIdx}`}>
