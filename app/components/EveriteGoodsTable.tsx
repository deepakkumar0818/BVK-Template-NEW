...
function quotationMultilineField(raw: Record<string, unknown> | undefined, key: string): string {
  if (!raw) return ''
  const v = raw[key]
  if (v == null) return ''
...
export default function EveriteGoodsTable({ data, rawQuotationData, shippingData, headerNode, footerNode }: EveriteGoodsTableProps) {
  ...
  const showPerformanceWarranty = performanceWarrantyText.length > 0
  const showPaymentSchedule = paymentScheduleText.length > 0
        ...
        return (
          ...
          >
            <div className="quotation-seamless-stack">
              ...
              <table
                className="goods-description-table quotation-stack-table everite-goods-table"
                style={{
                  width: '100%',
              ...
              >
                ...
                <tbody>
                  <tr className="everite-goods-title-row">
                  ...
                  {isLastChunk && (
                    <>
                      <tr aria-hidden className="everite-goods-spacer">
                      ...
                      {(showPerformanceWarranty || showPaymentSchedule) && (
                        <tr>
                          <td colSpan={2} style={{ ...bd, padding: '6px 10px', fontSize: '9px', verticalAlign: 'top', lineHeight: 1.25 }}>
                            ...
                            {showPaymentSchedule ? (
                              <>
                                <div style={{ fontWeight: 'bold', textDecoration: 'underline', marginBottom: '2px' }}>Payment Schedule :</div>
                                <div style={{ whiteSpace: 'pre-wrap' }}>{paymentScheduleText}</div>
                              </>
                            ) : null}
                          </td>
